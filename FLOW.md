...
└─────────────────────────┘
    ...
    │   └─ メールアドレスがある場合
    │       ├─ Gmailアドレス
    │       │   → 講師を登録（名前・LINE ID・メール）
    │       │   → 「登録OK：{氏名}先生」
    │       │
    │       └─ Gmail以外（iCloud等）
    │           → 講師を登録（名前・LINE IDのみ、メールは未登録）
    │           → 「登録OK：{氏名}先生」
    │           → 「※送信いただいたメールアドレスはGmailではないため登録できません。
    │              Gmailアドレスを送信してください。」
    │
    ├─ 複数見つかる
    │   → 「同じ氏名が複数います（候補：...）」
    ...
    └─ 新規登録（名簿にある講師）
        ...
        └─ メールアドレスがある場合
            ├─ Gmailアドレス → メールも登録
            └─ Gmail以外 → 「Gmailアドレスを送信してください」
```

**重要**: Googleスプレッドシートの編集権限にはGmailアドレスが必要なため、Gmail以外のメールアドレス（iCloud、Yahoo等）は登録できません。
